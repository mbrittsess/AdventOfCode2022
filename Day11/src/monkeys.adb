...
package body Monkeys is
   ...
   function Monkey_Business return Integer is
   ...
   begin
      ...
      while not This.Items.Is_Empty loop
         declare
            ...
            Divided : Worry := Altered/3;
            Test : Boolean := (Divided mod This.Test_Divisor) = 0;
         ...
         begin
            ...
            Log( "   Monkey gets bored with item. Worry level is divided by 3 to " & Divided'Image & "." );
            Log( "   Current worry is " & (if Test then "" else "not ") & "divisible by " & This.Operation_Amt'Image & "." );
            Log( "   Item with worry level " & Divided'Image & " is thrown to monkey " & Throw_To'Image & "." );

            This.Inspection_Count := This.Inspection_Count + 1;
            This.Items.Delete( This.Items.First_Index );
            Monkeys(Throw_To).Items.Append( Divided );
         end;
      end loop;
      Log( "  Inspection Times: " & This.Inspection_Count'Image & "." );
   end Do_Monkey_Turn;
